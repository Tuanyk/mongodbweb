...
function home_view() {    
    global $leodb, $database_name, $collection_name, $site_lang, $site_title, $site_desc, $site_url;
...
function document_view(string $slug) {
    global $leodb, $database_name, $collection_name, $site_lang, $site_url;
    $collection = $leodb->selectDatabase($database_name)->selectCollection($collection_name);
    $document = $collection->findOne(['slug'=>$slug]);
    $field_names = array_keys((array) $document);
    $title = $document['title'];
    $description = $document["title_$site_lang"];
    $canonical_url = $site_url.'/p/'.$document['slug'];
